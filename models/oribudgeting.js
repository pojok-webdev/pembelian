...
getSubmissionDetails = obj => {
    sql = 'select '
    sql+= ' a.id, a.submission_id, a.product_id, a.vendor_id, a.itemname, a.brand, a.partnumber, '
    sql+= ' a.description, a.amount, a.discountlevel, a.proposed_vendor, '
    sql+= ' format(a.final_price,2)final_price,'
    sql+= ' case when a.proposed_price is null then 0 else format(a.proposed_price,2) end proposed_price, '
    sql+= ' case when a.proposed_totalprice is null then 0 '
    sql+= ' else format(a.proposed_totalprice,2) '
    sql+= ' end proposed_totalprice, '
    sql+= ' a.vendor,d.nopo,d.scanpo,'
    sql+= ' case when a.final_price is null then (a.totalprice-a.proposed_totalprice) '
    sql+= ' else (a.final_price-a.proposed_totalprice) '
    sql+= ' end  difference,'
    sql+= ' case when a.price is null then 0 else format(a.price,2) end price , '
    sql+= ' case when a.totalprice is null then 0 else format(a.totalprice,2) end totalprice , '
    sql+= ' a.information, a.purchase_reason, a.placement_location, '
    sql+= ' a.guarantee, a.ppn, a.ongkir, a.note, '
    sql+= 'date_format(a.purchase_date,"%d %b %Y")purchase_date, a.reject_reason, '
    sql+= 'a.createuser, a.createdate, a.status, '
    sql+= 'date_format(a.arrivaldate,"%d %b %Y")arrivaldate,format(totalprice,2)totalprice,'
    sql+= 'b.category_id, '
    sql+= 'case a.placement_location '
    ...
    sql+= 'end category_name, '
    sql+= '(case '
    sql+= ' when c.amount is null then a.final_price-0 '
    sql+= ' when a.final_price>c.amount then a.final_price-c.amount '
    sql+= ' else 0 end) sisa, '
    sql+= '(case '
    sql+= ' when c.amount is null then 0 '
    sql+= ' when a.final_price<c.amount then c.amount-a.final_price '
    sql+= ' else 0 end) lebihbayar, '
    sql+= 'c.amount totalpayment, '
    sql+= 'd.subject,d.budgeting_number,date_format(d.submission_date,"%d %b %Y")submission_date,'
    sql+= 'date_format(d.implementation_target,"%d %b %Y")implementation_target,'
    sql+= 'date_format(purchase_target,"%d %b %Y")purchase_target,'
    sql+= 'd.staff_name,d.user_id '
    sql+= 'from submission_details a '
    sql+= 'left outer join products b on b.id=a.product_id '
    sql+= 'left outer join '
    sql+= '('
