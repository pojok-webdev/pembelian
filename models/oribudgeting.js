...
getSubmissionDetails = obj => {
    sql = 'select '
    sql+= ' a.id, a.submission_id, a.product_id, a.vendor_id, a.itemname, a.brand, a.partnumber, '
    sql+= ' a.description, a.amount, a.discountlevel, a.proposed_vendor, format(a.final_price,2)final_price,'
    sql+= ' case when a.proposed_price is null then 0 else format(a.proposed_price,2) end proposed_price, '
    sql+= ' case when a.proposed_totalprice is null then 0 else format(a.proposed_totalprice,2) end proposed_totalprice, '
    ...
    sql+= 'date_format(a.purchase_date,"%d %b %Y")purchase_date, a.reject_reason, a.createuser, a.createdate, a.status, '
    sql+= 'b.category_id, '
    ...
    sql+= '(case when c.amount is null then 0  when a.final_price<c.amount then c.amount-a.final_price else 0 end) lebihbayar, '
    sql+= 'c.amount totalpayment, '
    sql+= 'd.subject,d.budgeting_number,date_format(d.submission_date,"%d %b %Y")submission_date,date_format(d.implementation_target,"%d %b %Y")implementation_target,date_format(purchase_target,"%d %b %Y")purchase_target,d.staff_name,d.user_id '
    sql+= 'from submission_details a '
    sql+= 'left outer join products b on b.id=a.product_id '
    sql+= 'left outer join '
    sql+= '('
